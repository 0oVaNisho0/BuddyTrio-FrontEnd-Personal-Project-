...
function ButtonIBanking() {
  ...
  const handleCheckout = async (token) => {
    try {
      ...
      const resOrder = await addOrder({ address: '1234', orderItems });

      await deleteAllCart(user.id);
      ...
      if (res.data.status === 'successful') {
        await updateUserOrder({
          status: 'SUCCESS',
          orderId: resOrder.data.order.id,
        });
      }
